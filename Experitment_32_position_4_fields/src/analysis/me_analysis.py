...
def perform_elastic_analysis(time, data_baseline_subtracted, N_tau=N_tau_default, 
                        alpha=reg_par_ps_default, tau_min=tau_min_default, 
                        tau_max=tau_max_default, tau_sampling=tau_sampling_default,
                        weight=0.5):
    """
    Performs elastic net regularization (L1 + L2) for multiexponential decomposition.
    
    Parameters
    ----------
    time : array-like
        Time points
    data_baseline_subtracted : 2D array
        Data with baseline subtracted, shape (N_curves, N_time)
    N_tau : int
        Number of tau points
    alpha : float
        Regularization parameter
    tau_min, tau_max : float
        Min and max tau values
    tau_sampling : str
        'uniform' or 'non_uniform'
    weight : float, optional
        Weight between L1 and L2 (0 = pure L1, 1 = pure L2), default 0.5
    ...
    tau : array
        Relaxation times
    z : 2D array
        Fitted amplitudes, shape (N_curves, N_tau)
    data_fit : 2D array
        Fitted data, shape (N_curves, N_time)
    norms : array
    ...
    N_curves, N_time = data_baseline_subtracted.shape
    N_tau_actual = tau.shape[0]

    x0 = np.random.normal(0., 0.1, N_tau_actual)
    
    z = np.empty((N_curves, N_tau_actual))
    data_fit = np.zeros((N_curves, N_time))

    for i in range(N_curves):
        if (i % 10 == 0):
            logging.info(f"Processing curve {i}/{N_curves}")
        signal = data_baseline_subtracted[i, :]
        
        # Define objective and gradient for this signal
        def obj(g):
            return __opt(Phi, g, signal, alpha, weight)
        
        def jac(g):
            return __j(Phi, g, signal, alpha, weight)
        
        res = optimize.minimize(obj, x0, method='BFGS', jac=jac, tol=1e-9, 
                    options={'disp': False})
        z[i] = res.x
        data_fit[i, :] = np.dot(z[i, :], Phi)
    
    logging.info("Elastic net analysis completed.")
    return tau, z, data_fit, norms
...
def __opt(Phi, g, signal, reg_par, weight):
    """
    Objective function for elastic net regularization.
    Combines L2 (ridge) and L1 (lasso) penalties.
    
    Parameters
    ----------
    Phi : 2D array
        Basis matrix, shape (N_tau, N_time)
    g : 1D array
    ...
    reg_par : float
        Regularization parameter
    weight : float
        Weight between L2 and L1 (0=pure L1, 1=pure L2)
    """
    residual = Phi.T @ g - signal
    data_term = np.linalg.norm(residual)**2
    l2_term = weight * np.linalg.norm(g)**2
    l1_term = (1 - weight) * np.linalg.norm(g, ord=1)
    return data_term + reg_par * (l2_term + l1_term)

def __j(Phi, g, signal, reg_par, weight):
    """
    Gradient of the elastic net objective function.
    ...
    reg_par : float
        Regularization parameter
    weight : float
        Weight between L2 and L1
    """
    residual = Phi.T @ g - signal
    data_grad = 2 * Phi @ residual
    l2_grad = weight * 2 * g
    l1_grad = (1 - weight) * np.sign(g)
    return data_grad + reg_par * (l2_grad + l1_grad)
