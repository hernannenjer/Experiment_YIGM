...
class Channel:
    ...
    def perform_multiexponential_analysis_l2(
        self,
        N_tau=N_tau_default,
        ...
        """
        L2-based decomposition.
        """
        if self.time is None or self.data_baseline_subtracted is None:
            logging.warning(f"[Channel {self.channel_num}] Not enough data/time for L2.")
            return None, None

        tau, M_l2, data_fit_l2, norms = perform_l2_analysis(
            time=self.time,
            data_baseline_subtracted=self.data_baseline_subtracted,
            N_tau=N_tau,
            alpha=alpha,
            tau_min=tau_min,
            tau_max=tau_max,
        ...
        self.amplitudes = M_l2
        self.data_fit = data_fit_l2
        return self.relaxation_times, self.amplitudes

    def perform_multiexponential_analysis_fixed_taus(
        self,
        ...
        """
        3-exponential fit with known relaxation times, optional baseline.
        """
        if self.time is None or self.data_baseline_subtracted is None:
            logging.warning(f"[Channel {self.channel_num}] Not enough data/time for fixed_taus.")
            return None, None
        ...
        if fit_baseline and params.shape[1] == 4:
            self.baseline = params[:, 0]
            self.amplitudes = params[:, 1:]
        else:
            self.baseline = None
            self.amplitudes = params

        self.relaxation_times = np.array(approx_taus)
        self.data_fit = fitted_data
...
#             logging.info(f"Size of unparsed data = {self.unparsed_channel_data.shape}")
    ...
    def subtract_baseline(self, method='median', **kwargs):
        ...
        if method == 'mean_end':
            ...
            result = subtract_baseline_3points(self.raw_data)
        elif method == 'scalar':
