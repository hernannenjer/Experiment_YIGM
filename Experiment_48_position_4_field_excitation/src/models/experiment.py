...
class Experiment:
    ...
    def _subtract_baseline_standard(self, method='median', **kwargs):
        baselines = {}
        for (sample,key) in zip(self.samples.values(),self.samples.keys()):
            sample_baselines = sample.subtract_baseline_from_all_attempts(method=method, **kwargs)
            # print(key, sample_baselines)
            baselines[key]=sample_baselines
        # print(baselines)


        # Sort keys numerically (1, 2, 3, ..., 16)
        sorted_keys = sorted(baselines.keys(), key=lambda x: int(x))
        
        # Initialize lists for each channel
        ch_0, ch_1, ch_2 = [], [], []
        
        # Populate lists in sorted key order
        for key in sorted_keys:
            ch_0.append(baselines[key][0])
            ch_1.append(baselines[key][1])
            ch_2.append(baselines[key][2])
        
        # Convert to NumPy arrays
        baselines_0 = np.array(ch_0) # I am considering this 
        baselines_1 = np.array(ch_1)
        baselines_2 = np.array(ch_2)
        
        return baselines_0, baselines_1, baselines_2
    ...
    def subtract_channels_in_all_samples(self, channel_num_1, channel_num_2, new_channel_num=None):
        print('Subtracting the channel in all samples')
        for sample in self.samples.values():
    ...
    def perform_multiexponential_analysis_all_samples(
        ...
        if method == 'l2':
            for sample in self.samples.values():
                sample.perform_multiexponential_analysis_all_attempts_l2(
        ...
        elif method == 'fixed_taus':
            if not approx_taus:
                logging.warning("approx_taus must be specified for 'fixed_taus'.")
                return
            for sample in self.samples.values():
                sample.perform_multiexponential_analysis_all_attempts_fixed_taus(
                    channel_num=channel_num,
        ...
        elif method == 'fixed_taus_baseline':
            if not approx_taus:
                logging.warning("approx_taus must be specified for 'fixed_taus_baseline'.")
                return
            for sample in self.samples.values():
                sample.perform_multiexponential_analysis_all_attempts_fixed_taus(
                    channel_num=channel_num,
    ...
    def plot_baseline_vs_sample(self, log_axes=True, ch_num=None):
        plot_baseline_vs_sample(self, log_axes, ch_num)
