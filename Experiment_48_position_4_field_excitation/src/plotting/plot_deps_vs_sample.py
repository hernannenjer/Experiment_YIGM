...
def plot_peak_amplitudes_vs_sample(
    ...
    # -- ВСТАВКА: вычитаем значение в первой точке, если включен лог-масштаб и фит --
...
#         amplitudes = amplitudes - min_amp
        ...
        # Чтобы избежать отрицательных или нулевых значений после вычитания
        min_amp = np.min(amplitudes)
        if min_amp <= 0:
...
def plot_baseline_vs_sample(
    ...
    # --- Собираем baseline со всех каналов ---
    for sample_name, sample in experiment.samples.items():
        for attempt in sample.attempts:
            for channel in attempt.channels:
                # 1) Фильтр по номеру канала
                if ch_num is not None and channel.channel_num != ch_num:
                    continue

                # 2) Проверяем, что channel.baseline вообще есть (не None)
                if getattr(channel, 'baseline', None) is None:
                    continue

                # Готовим структуру хранения
                ...
                # Преобразуем имя сэмпла в число, если возможно
                try:
                    sample_prop = float(sample_name)
                except ValueError:
                    sample_prop = sample_name

                # 3) Если baseline - это массив (по кривым)
                #    то на каждую кривую один и тот же sample_prop.
                #    Если baseline - скаляр, добавим просто одну точку.
                baseline_val = channel.baseline
                if isinstance(baseline_val, np.ndarray):
                    # Допустим, baseline_val.shape = (Ncurves,)
    ...
    for c_num, data in baselines_per_channel.items():
        ...
        # Проверяем, можем ли мы ось X интерпретировать как числовую
        use_numeric_axis = True
        try:
        ...
        # Сгруппируем точки с одинаковым x (например, несколько кривых одного sName)
        ...
        # --- Фит, если надо ---
        if fit and use_numeric_axis:
            # Берём только положительные точки (если хотим power-law на log scale)
            if log_axes:
